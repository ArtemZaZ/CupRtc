...
/*
*Режим 1 (перелевание радугой всех светодиодов)
*/
void Mode_1(void){
	static int H = 0;
	H++;
...
*Внутрення функция заполнения массива для групп светодиодов
*/
void Mode_2_buffer_fill(int H1, int H2, int H3, int H4, int V){
	int i = 0;
...
/*
*Режим 2 (перелевание разной радугой по группам)
*/
void Mode_2(void){
...
*Внутрення фукнция сдвига эл-ов массива в третьем режиме
*/
void Shift_Led(void){
...
/*
*Режим 3 (бегающая радуга по всем светодиодам)
*/
void Mode_3(void){
	int H = 0;
	...
	for(int i = 0; i < WS2812B_NUM_LEDS; i++){  //на каждой новой итерации делаем сдвиг эл-ов
		while(!ws2812b_is_ready());  //массива на одну позицию
		ws2812b_send();
...
/*
*НЕ ДОДЕЛАН
*на данном этапе freertos-ом используются только три режима
*бегающая радуга по всем светодиодам
*/
